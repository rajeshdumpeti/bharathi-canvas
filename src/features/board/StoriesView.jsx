...
function readBoard(key, fallback) {
    try {
        const v = storage.get(BOARD_NS, key, fallback);
        return v ?? fallback;
    } catch {
        return fallback;
    }
...
export default function StoriesView() {
    const [tasks, setTasks] = useState(() => readBoard('tasks', []));
    const [projects, setProjects] = useState(() => readBoard('projects', []));
    const [selectedProjectId, setSelectedProjectId] = useState(() => readBoard('selectedProjectId', ''));
    const [q, setQ] = useState('');
    const [status, setStatus] = useState('all');

    // refresh from storage when page mounts
    useEffect(() => {
        setTasks(readBoard('tasks', []));
        setProjects(readBoard('projects', []));
        setSelectedProjectId(readBoard('selectedProjectId', ''));
    }, []);
    ...
    const filtered = useMemo(() => {
        ...
        return tasks
            .filter((t) => {
                ...
                const hay = `${t.storyId || ''} ${t.title || ''} ${t.assignee || ''} ${t.acceptanceCriteria || ''}`.toLowerCase();
        ...
        <div className="h-screen w-full flex flex-col bg-gray-50">
            <div className="bg-white border-b">
                <div className="mx-auto w-full max-w-7xl flex items-center justify-between px-6 py-4">
                    ...
                    <Link to="/board" className="px-3 py-1.5 rounded border bg-white hover:bg-gray-50 text-gray-700">Back to Board</Link>
                </div>
            </div>
                ...
                <div className="mx-auto w-full max-w-7xl p-6 space-y-4">
                    ...
                    <div className="overflow-auto rounded border bg-white">
                        <table className="min-w-full text-sm">
                            ...
                            <tbody>
                                {filtered.length === 0 ? (
                                    <tr><td colSpan={6} className="p-6 text-center text-gray-500">No stories match.</td></tr>
                                ) : filtered.map(t => (
                                    <tr key={t.id} className="border-t">
                                        <td className="p-3 font-mono text-blue-700">{t.storyId || '—'}</td>
                                        <td className="p-3">
                                        ...
                                        <td className="p-3">
                                            {(() => {
                                                ...
                                                const color = STATUS_COLORS[t.status] || 'bg-gray-200 text-gray-800';
                                                return (
                                        ...
                                        <td className="p-3">{(t.createdAt || '').slice(0, 10) || '—'}</td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
